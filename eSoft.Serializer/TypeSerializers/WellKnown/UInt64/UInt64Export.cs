...
namespace eSoft.Serializer
{
    ...
    {
        // Serialization of UInt64
        public static byte[] SerializeUInt64(UInt64 valueToSerialize)
        {
            ...
            // Encode value
            byte[] encodedValue = BitToolkit.ConvertInt64ToByteArray((Int64)valueToSerialize);
        ...
        public static UInt64 DeserializeUInt64(byte[] serializedData)
        {
            // 1 or 2 bits for config (lsb)
            // x1 - stored in this byte (on rest 7 bits)
            // 00 - has default value 0
            ...
            switch (configCase)
            {
                case 2: // 10 - stored in next few bytes - then next 2 bits says on how many bytes the value has been stored
                    return (UInt64)BitToolkit.ConvertByteArrayToInt64(serializedData, 1);
