...
namespace eSoft.Serializer
{
    ...
    {
        // Serialization of UInt32
        public static byte[] SerializeUInt32(UInt32 valueToSerialize)
        {
            ...
            // Encode value
            byte[] encodedValue = BitToolkit.ConvertInt32ToByteArray((Int32)valueToSerialize);
        ...
        public static UInt32 DeserializeUInt32(byte[] serializedData)
        {
            // 1 or 2 bits for config (lsb)
            // x1 - stored in this byte (on rest 7 bits)
            // 00 - has default value 0
            ...
            switch (configCase)
            {
                case 2: // 10 - stored in next few bytes - then next 2 bits says on how many bytes the value has been stored
                    return (UInt32)BitToolkit.ConvertByteArrayToInt32(serializedData, 1);
